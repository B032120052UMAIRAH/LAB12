...
public class RainFallByteBasedReader {
	public static void main(String[] args) {
		...
		try {
			...
			//HashSet to store distinct district for calculation purpose(prevent duplicate)
			...
			for(int index = 0; index < stations.size(); index++)
			{
				...
				System.out.println("Total daily rainfall readings in " + stations.get(index) + " : " 
						+ totalRainfall.get(index) );
				System.out.println("Average daily rainfall readings in " + stations.get(index) + " : " 
						+ formattedAverage + "\n");
			}
			
			System.out.println("\nThere are " + stations.size() + " stations and "
